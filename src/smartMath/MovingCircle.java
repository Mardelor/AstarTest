...
public class MovingCircle extends Circle {
    ...
    public MovingCircle(Vector center, int ray){
        super(center, ray);
        speedVector = new Vector(TestMode.initialSpeed, MathLib.randomGaussian(MathLib.modulo(center.getTheta() - Math.PI, 2*Math.PI), Math.PI/8));
    }
    ...
    public void updatePosiion(){
        this.center.addVector(speedVector);
        this.speedVector = new Vector(MathLib.randomGaussian(speedVector.getRay(), 1), MathLib.randomGaussian(speedVector.getTheta(), Math.PI/8));
    }
